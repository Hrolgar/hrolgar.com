...
export default function Experience({ experience, heading, resumeUrl, locale = DEFAULT_LOCALE }: Props) {
            ...
            <ScrollReveal key={exp._id} delay={index * 30}>
              <div className="py-4 border-b border-border last:border-b-0 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-1 sm:gap-4">
                ...
                <span className="text-sm text-muted whitespace-nowrap flex-shrink-0">
                  {formatMonthYear(exp.startDate, locale)} – {exp.endDate ? formatMonthYear(exp.endDate, locale) : t("present", locale)}
                </span>
              </div>
            </ScrollReveal>
          ))}
